...
t.ALL$res = paste0(t.ALL$pop,' - Ivermectin resistant')
t.ALL$res[t.ALL$pop=='AUS.2'] = paste0(t.ALL$pop[t.ALL$pop=='AUS.2'],' - Ivermectin susceptible')

## Retain coordinates of interest 
piaex3 = t.ALL[t.ALL$Chr=='1_Celeg_TT' & t.ALL$WinCenter>9597364-50e3 & t.ALL$WinCenter<9597364+50e3,] 
...
p1 = ggplot(piaex3,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_manual(values = c('#e31a1c','#41b6c4'))+ 
  ...
  theme(legend.position = 'none') +
  ggtitle('HCOI00489500 (aex-3 ortholog)')

p2 = ggplot(piglc4,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_manual(values = c('#e31a1c','#41b6c4'))+ 
  geom_point(size=3,alpha=.4) + geom_line() +
  scale_x_continuous() +
  facet_wrap(~ res) +
...
p3 = ggplot(pipgp11,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_manual(values = c('#e31a1c','#41b6c4'))+ 
  geom_point(size=3,alpha=.4) + geom_line() +
  scale_x_continuous() +
  facet_wrap(~ res) +
...
pdf(file=paste0(draft2,'Figure3d.pdf'),width=8,height=8)
multiplot(p1,p2,p3)
dev.off()

t.test(tP/nSites ~ res,data = piaex3)
# Welch Two Sample t-test
# 
# data:  tP/nSites by res
# t = 2.1343, df = 17.874, p-value = 0.04693
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   2.352859e-05 3.085071e-03
# sample estimates:
#   mean in group IVM-R mean in group Unknown 
# 0.01158518            0.01003088

t.test(tP/nSites ~ res,data = piglc4)
# Welch Two Sample t-test
# 
# data:  tP/nSites by res
# t = 4.8635, df = 10.804, p-value = 0.0005264
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.002694300 0.007166887
# sample estimates:
#   mean in group IVM-R mean in group Unknown 
# 0.008280649           0.003350055 

t.test(tP/nSites ~ res,data = pipgp11)
# Welch Two Sample t-test
# 
# data:  tP/nSites by res
# t = 7.3753, df = 10.471, p-value = 1.848e-05
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.001736229 0.003226387
# sample estimates:
#   mean in group IVM-R mean in group Unknown 
# 0.007083313           0.004602005 
...
t.ALL$res = paste0(t.ALL$pop,' - Ivermectin susceptible')
t.ALL$res[t.ALL$pop=='STA.1'] = paste0(t.ALL$pop[t.ALL$pop=='STA.1'],' - Ivermectin resistant')
...
p4=ggplot(piaex32,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
...
p5=ggplot(pisnf9,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_brewer(palette = 'YlGnBu')+ 
  geom_point(size=3,alpha=.4) + geom_line() +
  scale_x_continuous() +
  #facet_wrap(~ res) +
...
p6=ggplot(piunc24,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_brewer(palette = 'YlGnBu')+ 
  geom_point(size=3,alpha=.4) + geom_line() +
  scale_x_continuous() +
  #facet_wrap(~ res) +
...
p7=ggplot(pib,aes(x=WinCenter/1000,y=tP/nSites,col=pop)) +
  scale_color_brewer(palette = 'YlGnBu')+ 
  geom_point(size=3,alpha=.4) + geom_line() +
  scale_x_continuous() +
  #facet_wrap(~ res) +
...
multiplot(p4,p5,p6,p7,p1,p2,p3,cols=2)
dev.off()
...
##- get universal region boundaries
